...
# Create
@app.route('/fcallback', methods=['POST'])
def fcallback():
    # Validate state token
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter.'), 401)
    ...
    # Obtain authorization code
    code = request.data
    # Exchange client token (code) for a long-lived token with get
"""""
...
# Create function to check disconnect facebook or google
...
def disconnect():
    if 'provider' in login_session:
        ...
        redirect(url_for('restaurantName'))
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        return """<script>function myFunction() {alert('You are not authorized to edit this item.
                You are not the owner of this restaurant');}</script><bodyonload='myFunction()''>"""
    if request.method == 'POST':
